...
log_label = tk.Label(frame, text="Paste your travel log (tab-separated, with headers):")
log_label.pack(anchor="w")

travel_log_text = scrolledtext.ScrolledText(frame, width=80, height=18)
travel_log_text.pack(fill=tk.BOTH, expand=True)

as_of_date_label = tk.Label(frame, text="Enter 'as of' date (YYYY-MM-DD):")
as_of_date_label.pack(anchor="w", pady=(10,0))

as_of_date_entry = tk.Entry(frame, width=20)
as_of_date_entry.pack(anchor="w")

calc_button = tk.Button(frame, text="Calculate Days", command=calculate_days_gui)
